import { RADIO_PWA_INSTALL_READY_EVENT } from './constants';
...
/** Inline script — registers before React hydrates when install intent is already active. */
export const RADIO_PWA_INSTALL_EARLY_CAPTURE_SCRIPT = `(function(){function i(){return document.cookie.indexOf("idling-radio-pwa-install-intent=1")!==-1||localStorage.getItem("idling-radio-pwa-install-intent")==="1"}if(!i())return;window.addEventListener("beforeinstallprompt",function(e){if(!i())return;e.preventDefault();window.__idlingRadioDeferredPrompt=e;window.dispatchEvent(new CustomEvent("idling-radio-install-ready"));});})();`;
