import type { ParseRadioPlaylistInput, ParseRadioPlaylistResult } from './radioStreamFormat.types';

function resolvePlaylistUrl(rawUrl: string, baseUrl: string): string {
  try {
...
function parsePlsPlaylist(content: string, baseUrl: string): ParseRadioPlaylistResult {
...
function parseM3uPlaylist(content: string, baseUrl: string): ParseRadioPlaylistResult {
  const lines = content.split(/\r?\n/);
...
export function parseRadioPlaylist(input: ParseRadioPlaylistInput): ParseRadioPlaylistResult {
