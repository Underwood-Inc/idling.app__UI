...
function envelopeCoeff(deltaSeconds, timeConstantSeconds) {
    return 1 - Math.exp(-deltaSeconds / Math.max(timeConstantSeconds, 0.001));
}
export function buildFrequencyBarTargets(input) {
    const targets = new Float32Array(input.barCount);
    fillFrequencyBarTargets(targets, input);
    return targets;
}
export function fillFrequencyBarTargets(out, input) {
...
export function createBarLevelEnvelope(count, options = {}) {
