...
function resolveBarFill(drawContext, level, index, total) {
...
function drawFrequencyBars(drawContext) {
    ...
    forEachBar(drawContext, (index, level, x, barW) => {
        if (barTrail === 'peaks') {
            ...
            trailGradient.addColorStop(0, resolveBarFill(drawContext, state.peaks[index], index, drawContext.data.length));
            trailGradient.addColorStop(1, resolveBarFill(drawContext, level * 0.35, index, drawContext.data.length));
...
function buildWavePoints(drawContext) {
...
function traceAngularWavePath(ctx, points) {
    if (points.length === 0) {
        return;
    }
...
function traceRibbonWavePath(ctx, points) {
    if (points.length < 2) {
        return;
    }
    if (points.length === 2) {
...
function drawScope(drawContext) {
    paintCanvasBg(drawContext);
...
function drawDriftMist(drawContext) {
    paintCanvasBg(drawContext);
    const { ctx, width, height, data, theme, state, fullscreen } = drawContext;
    const layerCount = fullscreen ? 6 : 5;
    advanceAudioStreamTempoPhase(state, drawContext.tempo, fullscreen ? 0.005 : 0.007, drawContext.deltaSeconds);
...
function drawOrbitSwarm(drawContext) {
    paintCanvasBg(drawContext);
    const { ctx, data, theme, state } = drawContext;
    const { cx, cy, extent } = resolveCircularVisualizerMetrics(drawContext);
    advanceAudioStreamTempoPhase(state, drawContext.tempo, 0.007 + (data[0] ?? 0) * 0.012, drawContext.deltaSeconds);
    let energy = 0;
    for (let index = 0; index < data.length; index += 1) {
        energy += data[index] ?? 0;
    }
    energy /= Math.max(1, data.length);
    const baseR = extent * 0.14;
    ctx.strokeStyle = rgba(theme.primary, 0.12 + energy * 0.2);
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.arc(cx, cy, baseR + energy * extent * 0.18, 0, Math.PI * 2);
    ctx.stroke();
    for (let index = 0; index < data.length; index += 3) {
        ...
        const angle = (index / data.length) * Math.PI * 2 + state.phase;
        const orbitR = baseR + level * extent * 0.34;
        const px = cx + Math.cos(angle) * orbitR;
        const py = cy + Math.sin(angle) * orbitR;
        const dotR = 1.2 + level * 4.5;
        const glow = ctx.createRadialGradient(px, py, 0, px, py, dotR * 2.2);
        glow.addColorStop(0, rgba(theme.primary, 0.45 + level * 0.4));
        glow.addColorStop(1, rgba(theme.secondary, 0));
        ctx.fillStyle = glow;
        ctx.beginPath();
...
function fadeOrganicCanvas(drawContext, fadeAlpha) {
...
function drawThreadWeave(drawContext) {
    paintCanvasBg(drawContext);
    ...
    for (let thread = 0; thread < threadCount; thread += 1) {
        ...
        const stroke = drawContext.colorPalette === 'prism'
...
function resolveCircularVisualizerMetrics(drawContext) {
...
function drawPulseBlob(drawContext) {
    ...
    energy /= Math.max(1, data.length);
    advanceAudioStreamTempoPhase(state, drawContext.tempo, 0.014 + energy * 0.028, drawContext.deltaSeconds);
    const { cx, cy, extent } = resolveCircularVisualizerMetrics(drawContext);
    const baseR = extent * 0.18;
    const pulseR = baseR + energy * extent * 0.28;
...
function drawHaloRing(drawContext) {
    paintCanvasBg(drawContext);
    const { ctx, data, theme, state } = drawContext;
    let bassEnergy = 0;
    const bassBins = Math.max(1, Math.floor(data.length * 0.22));
    for (let i = 0; i < bassBins; i += 1) {
        bassEnergy += data[i] ?? 0;
    }
    bassEnergy /= bassBins;
    advanceAudioStreamTempoPhase(state, drawContext.tempo, 0.016 + bassEnergy * 0.02, drawContext.deltaSeconds);
    const { cx, cy, extent } = resolveCircularVisualizerMetrics(drawContext);
    const baseR = extent * 0.22;
    const ringR = baseR + bassEnergy * extent * 0.2;
    const thickness = 2 + bassEnergy * 7;
    ctx.strokeStyle = rgba(theme.primary, 0.25 + bassEnergy * 0.55);
    ctx.lineWidth = thickness;
    ctx.beginPath();
    ctx.arc(cx, cy, ringR + Math.sin(state.phase) * 2, 0, Math.PI * 2);
    ctx.stroke();
    ctx.strokeStyle = rgba(theme.secondary, 0.15 + bassEnergy * 0.35);
    ...
    for (let i = 0; i < data.length; i += 4) {
        ...
        const angle = (i / data.length) * Math.PI * 2 + state.phase * 0.15;
        const dotR = ringR + level * extent * 0.08;
        const dx = cx + Math.cos(angle) * dotR;
        const dy = cy + Math.sin(angle) * dotR;
        ctx.fillStyle = rgba(theme.secondary, 0.2 + level * 0.6);
        ctx.beginPath();
        ctx.arc(dx, dy, 1 + level * 2.2, 0, Math.PI * 2);
        ctx.fill();
    }
}
...
export const BAR_VISUALIZER_PRESET_DEFINITIONS = [
    ...
    },
    {
        id: 'orbit-swarm',
        label: 'Orbit swarm',
        description: 'Orbiting energy particles',
        dockLayout: 'compact',
        fullscreenLayout: 'radial',
        fullscreenOnly: true,
    },
    {
        id: 'arc',
    ...
    },
    {
        id: 'halo',
        label: 'Halo',
        description: 'Reactive ring field',
        dockLayout: 'compact',
        fullscreenLayout: 'radial',
        fullscreenOnly: true,
    },
    {
        id: 'pulse',
        label: 'Pulse',
        description: 'Breathing energy core',
        dockLayout: 'compact',
        fullscreenLayout: 'radial',
        fullscreenOnly: true,
    },
...
const PRESET_DRAWERS = {
    ...
    'orbit-swarm': drawOrbitSwarm,
    arc: drawArcSpectrum,
    halo: drawHaloRing,
    pulse: drawPulseBlob,
};
export function normalizeBarVisualizerPresetId(presetId) {
...
export function isFrequencyBarsVisualizerPreset(presetId) {
    return normalizeBarVisualizerPresetId(presetId) === FREQUENCY_BARS_PRESET_ID;
}
export function isBarVisualizerFullscreenOnly(presetId) {
    const normalized = normalizeBarVisualizerPresetId(presetId);
    return (BAR_VISUALIZER_PRESET_DEFINITIONS.find((preset) => preset.id === normalized)?.fullscreenOnly ===
        true);
}
export function isBarVisualizerDockOnly(presetId) {
    const normalized = normalizeBarVisualizerPresetId(presetId);
    return (BAR_VISUALIZER_PRESET_DEFINITIONS.find((preset) => preset.id === normalized)?.dockOnly === true);
}
export function getBarVisualizerFullscreenLayout(presetId) {
    const normalized = normalizeBarVisualizerPresetId(presetId);
    const preset = BAR_VISUALIZER_PRESET_DEFINITIONS.find((entry) => entry.id === normalized);
    return preset?.fullscreenLayout ?? 'strip';
}
export function isScopeVisualizerPreset(presetId) {
    return normalizeBarVisualizerPresetId(presetId) === 'scope';
}
export function isBarVisualizerCircularPreset(presetId) {
    const layout = getBarVisualizerFullscreenLayout(presetId);
    return layout === 'radial' || layout === 'hemisphere';
}
export function listBarVisualizerPresetsForSurface(surface) {
    if (surface === 'expanded') {
...
export function getBarVisualizerPresetDefinition(presetId) {
    const normalized = normalizeBarVisualizerPresetId(presetId);
    return (BAR_VISUALIZER_PRESET_DEFINITIONS.find((preset) => preset.id === normalized) ??
